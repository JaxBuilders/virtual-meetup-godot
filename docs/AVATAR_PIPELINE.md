...
The clothed MPFB proof is the local and remote gameplay visual. Humanoids must use sourced assets, including gallery and customization previews. Procedural generation/modification of sourced humans is allowed; assembled primitive humans and accessories are not. Invalid catalog IDs fall back to supported authored selections. A missing/corrupt bundled avatar or animation library reports an asset error rather than constructing a primitive substitute. Invisible collision shapes are unaffected. Idle, walk, jog, jump, sitting, dance, holding, and throwing use the approved Quaternius library; wave, point, and clap temporarily use its generic `Interact` gesture until dedicated clips are authored or approved. Body presets apply conservative uniform scale differences.

Home and room customization share an isolated 3D preview of the same gameplay avatar. Selection changes apply, persist, and publish through the existing profile flow immediately, without an Apply button. The current wardrobe offers five suit color tints (including charcoal and cream), six skin tints, and sourced cropped hair or bald. Cap/bun/glasses/badge options are retired until sourced replacements are integrated: old hair IDs normalize to crop and accessory IDs to none. Appearance updates preserve animation playback and use per-instance material overrides. These are variants of one fitted outfit, not separate authored garment sets.
