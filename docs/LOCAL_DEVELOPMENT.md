...
Launch **Virtual Meetup: Blender + MCP + MPFB** in VS Code for the project authoring workspace, or use the plain Blender entry without session add-ons. The harness uses `BLENDER_BIN` first, then `blender` on `PATH`, and reports the actual version without replacing your installation. `node dev/dev.mjs blender --check` verifies discovery without opening a window; `node dev/dev.mjs blender path/to/avatar.blend` opens a source file. Paths containing spaces must be quoted. The authoring launcher sets `DISABLE_TELEMETRY=true`, registers the pinned local MPFB and Blender MCP snapshots for that session, disables external asset integrations, and exposes the ignored approved wardrobe library when present. See `docs/BLENDER_SETUP.md` for installation and validation details.
